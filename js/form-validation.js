function validateForm(button) {
    ...
    if ($firstError != null) {
        ...
        $('body').animate({ scrollTop: ($firstError.closest('.ml-val-container').position().top - 100) });
        return false;
    } else
        return true;
}
...
function validateInputField($field) {
    ...
    if (required || $field.val().length > 0) {
        ...
        if ($field.is('[ml-val-function]'))
            customValidation = $field.attr('ml-val-function');

        var msg = "";

        if (type == 'text') {
            msg = validateText($field.val(), label, min, customValidation);
        }
        if (type == 'number') {
            msg = validateNumber($field.val(), label, format, max, min, customValidation);
        }
        if (type == 'date') {
            msg = validateDate($field.val(), label, max, min, customValidation);
        }
        if (type == 'time') {
            msg = validateTime($field.val(), label, max, min, customValidation);
        }
        if ($field.is('[type=checkbox]')) {
            msg = validateCheckbox($field, label, customValidation);
        }
        if (msg.length > 0) {
            validated = false;
...
function validateCheckbox($field, label, customValidation) {
    var error = "";
    var required = false;
    if ($field.is('[ml-val-required]'))
        required = $field.attr('ml-val-required') == 'true';
    ...
    if (error.length == 0) {
        if (customValidation.length > 0)
            error += eval(customValidation);
    }

    return error;
}
function validateText(value, label, minLength, customValidation) {
    var error = "";
    value = value.trim();
    if (value.length == 0) {
...
function validateNumber(value, label, type, maximum, minimum, customValidation) {
    var error = "";
    if (type == "integer") {
        ...
        console.log('validateNumber decimal');
...
function validateDate(value, label, maximum, minimum, customValidation) {
    var error = "";
...
function validateTime(value, label, maximum, minimum, customValidation) {
    var error = "";
    // First check for the pattern
